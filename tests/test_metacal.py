...
replicates galsim's drawFFT), so it adds no edge/aliasing artifact; the
intended difference is only that the transfer and the hybrid filter live on the
padded k-grid rather than a second small fft.
"""
...
def _build(theta):
    ...
    cen = (DIM - 1.0) / 2.0
    jac = (
        ngmix.DiagonalJacobian(row=cen, col=cen, scale=SCALE)
        if jacmat is None
        else ngmix_jacobian(cen, cen, jacmat)
    )
...
def test_metacal_matches_ngmix(theta):
    ...
    for img, mkobs in [(gal_im, obs), (noise, _noise_obs(obs, noise))]:
        ...
        for t in TYPES:
            a, b = od[t].image, ki[t]
            assert (
                np.sqrt(((a - b) ** 2).mean()) / np.sqrt((a**2).mean()) < 1e-6
            )
