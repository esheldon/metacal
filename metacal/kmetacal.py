...
transfer ``|K_t|^2`` and the filtered correction field are built WITHOUT a second,
aliasing real<->k round trip.  A single ``np.ifft2`` returns to real space.
...
``wcs.profileToImage(profile)`` at ``dk = 2*pi/Np`` samples exactly the spectral
grid a numpy ``fft2`` of that drawn image would -- on the axis-aligned
...
Padding
-------
The draw grid ``Np`` is galsim's own FFT draw size (``drawFFT_makeKImage``), not
the stamp size N: it gives the object's real-space tails room so the periodic
ifft does NOT wrap across the stamp.  ``Np`` is computed automatically and is NOT
a tuning knob; it is exposed only so a set of metacals (galaxy, noise, transfer)
can be forced onto ONE shared grid -- the filter and the noise it cancels must
live on a single frame.  The high-level ``metacal_hybrid`` does that threading.
...
+90 before metacal and the metacal'd field -90 after, so ``FT(corr) = T_M(R90 k)
N(k)`` -- the 90 lands on the analytic metacal transfer, the noise keeps its
...
def _shear_kwargs(t, step):
    ...
    g1=+/-step, 2p/2m shear g2=+/-step -- 2p/2m give the g2 column of the full 2x2
    response (needed for the trace response Rbar; only 1p/1m for the R11 path)."""
    return {
        'noshear': None,
        '1p': {'g1': step, 'g2': 0.0},
        '1m': {'g1': -step, 'g2': 0.0},
...
class KMetacal:
    ...
    Builds the persistent galsim GSObjects once (interpolated image, deconvolved
    galaxy, dilated round-gaussian target psf) and exposes the per-type metacal'd
    field as the matched-grid k-array (``get_khats``) or the real image
    (``get_images``).
    ...
    psf_image: (N, N) array
        the psf image (pixel-convolved, as drawn); deconvolved from the image and
        used to derive the round gaussian reconvolution target
    wcs: galsim local/Jacobian wcs
        the pixel->sky wcs; a diagonal scale or a distorted (rotated/sheared)
        jacobian, handled exactly via profileToImage
    step: float
        metacal shear step
    ...
    Np: int, optional
        ...
        size.  Pass an explicit Np ONLY to force one shared grid across a set of
        KMetacals (the filter and the noise it cancels must share a frame) -- not
        a tuning knob.
    rotation: galsim.Angle or None
        sky-frame rotation of the input field before metacal (the metacal'd field
        is rotated back by -rotation); ``90 * galsim.degrees`` builds the fixnoise
        correction field, None (default) is the identity (galaxy/science path)
    x_interpolant: str
        the InterpolatedImage interpolant (default lanczos15, the metacal kernel)
    """

    def __init__(self, image, psf_image, wcs, step=0.01,
                 types=DEFAULT_TYPES, Np=None, rotation=None,
                 x_interpolant=LANCZOS):
        if rotation is not None and not isinstance(rotation, galsim.Angle):
            raise TypeError(
                'rotation must be a galsim.Angle (e.g. 90 * galsim.degrees) or '
        ...
        img = galsim.Image(np.ascontiguousarray(image, dtype=float), wcs=wcs)
        ...
        pimg = galsim.Image(
            np.ascontiguousarray(psf_image, dtype=float), wcs=wcs)
    ...
    def _galsim_kpad_size(self):
        ...
        ``GSObject.drawFFT_makeKImage``: the good image size from stepk, at least
        the stamp size, rounded to a good FFT size, floored at minimum_fft_size.
        Set by the image size (stepk), so same-N images share it."""
    ...
    def _metacal_field(self, t):
        ...
        nopsf = (self.image_int_nopsf if sh is None
    ...
    def get_khats(self):
        ...
        if self._khats is None:
            self._khats = {t: self._khat(self._metacal_field(t))
                           for t in self.types}
        return self._khats
    ...
    def get_images(self):
        ...
        return {t: self._crop(np.fft.ifft2(k).real)
    ...
    def get_filtered_images(self, filters):
        ...
        ``filters`` are Np x Np k-space filters (the hybrid H_t).  The filter is
        applied in k on the padded grid -- no second fft -- so the de-aliased
        correction field stays in the same frame as the transfer that built it."""
        kh = self.get_khats()
        return {t: self._crop(np.fft.ifft2(kh[t] * filters[t]).real)
                for t in self.types}
    ...
    def target_psf_image(self):
        ...
        return self.target_psf.drawImage(
            nx=self.N, ny=self.N, wcs=self.wcs, method='no_pixel').array


def delta_transfer_kspace(psf_image, wcs, dim, step, types,
                          Np=None, rotation=None):
    """
    the per-type metacal noise transfer P_t = |K_t|^2 on the padded Np x Np grid,
    computed k-natively: push a unit delta through ``KMetacal`` and take
    ...
    km = KMetacal(delta, psf_image, wcs, step=step, types=types, Np=Np,
                  rotation=rotation)
    khats = km.get_khats()
    return {t: np.abs(khats[t]) ** 2 for t in types}, km.Np


def make_hybrid_filters_kspace(pts, pts_rot, npix, scale, types,
                               extra_iso=0.0, ktol=1e-4, jmat=None):
    """
    ...
    ``pts`` is the un-rotated image-metacal transfer (the noise to isotropize, in
    its own frame); ``pts_rot`` is the rotated-back corr-field transfer
    (|T_M(R90 k)|^2).  The common-level m=2/m=6 deficit D is built from ``pts``
    (sky-projected, trapz quadrature -- ``common_harmonic_deficits``) and divided
    by ``pts_rot``:
    ...
    jmat: (2, 2) array, optional
        the pixel->sky jacobian for the sky-angle deficit projection (None = the
        pixel frame = a diagonal wcs)
    """
    ...
    for t in types:
        ...
        eps = ktol * pt.max()
        hraw = np.sqrt(np.divide(padd, pt, out=np.zeros_like(padd),
                                 where=pt > 0))
...
def _wcs_matrix(wcs):
    """the 2x2 pixel->sky jacobian (col, row order) of a galsim local wcs"""
    jac = wcs.jacobian()
    return np.array([[jac.dudx, jac.dudy], [jac.dvdx, jac.dvdy]])
...
def metacal_hybrid(image, psf_image, wcs, noise_image,
                   step=0.01, types=DEFAULT_TYPES, extra_iso=0.0):
    """
    ...
    metacal noise power by adding a full counter-rotated, metacal'd noise field --
    it works but DOUBLES the variance.  Only the m=2/m=6 anisotropy needs
    cancelling, so the hybrid filters that same (sky-)rotated, metacal'd field
    down to just the anisotropic deficit before adding it: ~0.05x (round PSF) to
    ~0.5x (elliptical PSF) the added variance, vs 1.0x for fixnoise.  Because the
    added field is the ACTUAL rotated metacal'd noise it carries the genuine
    (possibly non-stationary) covariance with the cancelling sign for free.

    Everything is on ONE shared padded k-grid (the galaxy, the sky-rotated noise,
    and the transfers), with the winning choices baked in: world shear frame,
    galsim's automatic padding, the trapz azimuthal quadrature, the sky-angle
    deficit projection, and the SKY-frame 90-deg rotation of the correction field.
    ...
    psf_image: (N, N) array
        the psf image
    wcs: galsim local/Jacobian wcs
        the pixel->sky wcs (diagonal or distorted)
    noise_image: (N, N) array
        the correction noise field -- an independent realization matching the
        image's noise (its correlations / coverage); rotated, metacal'd, filtered
        and added to restore the noise symmetry
    step: float
        metacal shear step
    types: sequence of str
        metacal types (default noshear/1p/1m; add 2p/2m for the full 2x2 response)
    extra_iso: float
        extra isotropic deficit power (tunes toward fixnoise; 0 = minimal added
        variance)
    ...
    Returns
    -------
    dict type -> (N, N) corrected metacal image (the metacal'd science image plus
    the filtered, sky-rotated correction field)
    """
    dim = image.shape[0]
    scale = float(np.sqrt(abs(np.linalg.det(_wcs_matrix(wcs)))))
    jmat = _wcs_matrix(wcs)
